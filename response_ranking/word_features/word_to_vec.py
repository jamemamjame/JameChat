...
class word2vec():
    __sentence_proc = None
    # path of google word2vec
    __google_w2v_path = path.GOOGLE_WORD_TO_VEC
    ...
    def __init__(self):
        ...
        self.model = gensim.models.KeyedVectors.load_word2vec_format(self.__google_w2v_path,
                                                                     binary=True)
        self.__sentence_proc = Sentence_prepocess(keep_stopword=False)

    def get_score(self, query, candidate):
        '''
        ...
        candidate = self.__sentence_proc.cleaning(candidate)

        sum_cosine = 0.0
        for word1 in query:
            for word2 in candidate:
                # compute a cosine similarity between 2 word
                sum_cosine += self.model.similarity(word1, word2)
